...
class ControllerLoader
{
  ...
  public function resolve(Event $event)
  {
    ...
    $event->setReturnValue(
      $this->find($bundle, $controller, $action, $event->getParameter('request')->getPathParameters())
    );
  ...
  public function find($bundle, $controller, $action, array $parameters)
  {
    $class = null;
    $logs = array();
    ...
    foreach ($r->getMethod($method)->getParameters() as $param)
    {
      ...
      {
        throw new \RuntimeException(sprintf('Controller "%s:%s()" requires a "%s" argument.', $class, $method, $param->getName()));
      }
    }

    return array(array($controller, $method), $arguments);
  }
}
